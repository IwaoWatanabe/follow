...
public class FileFollower {
	...
	public FileFollower(File file, int bufferSize, int latency,
		...
		int initOutputDestsSize = (initialOutputDestinations != null) ? initialOutputDestinations.length
				: 0;
		outputDestinations = new ArrayList<OutputDestination>(
				initOutputDestsSize);
	...
	class Runner implements Runnable {
		private final Logger log = Logger.getLogger(Runner.class.getName());
		...
		protected void runAction() {
			try {
				...
				log.finer("exiting Runner.runAction [continueRunning="
						+ continueRunning + "; needsRestart=" + needsRestart
						+ "]");
				bis.close();
				fis.close();
			} catch (IOException e) {
