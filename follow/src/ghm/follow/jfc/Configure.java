...
import ghm.follow.jfc.ui.FontSelectionPanel;
import ghm.follow.jfc.ui.InvalidFontException;
...
class Configure extends FollowAppAction {
	...
	class CfgDialog extends JDialog {
		...
		CfgDialog() {
			super(Configure.this.getApp().getFrame(), FollowApp
			...
			configPanel
					.add(new JLabel(
							FollowApp
									.getResourceString("dialog.Configure.latency.label")),
							gbc);
			...
			gbc.ipadx = 4;
			configPanel
					.add(new JLabel(FollowApp
							.getResourceString("dialog.Configure.editor.label")),
							gbc);
			...
			configPanel
					.add(new JLabel(
							FollowApp
									.getResourceString("dialog.Configure.tabSize.label")),
							gbc);
			...
			// Save button
			save = new JButton(
					FollowApp.getResourceString("dialog.Configure.save.label"));
			save.setMnemonic(FollowApp.getResourceString(
			...
			save.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (!isPositiveInteger(bufferSize.getText())) {
						invalidFieldsMessage.append(FollowApp
								.getResourceString("dialog.Configure.bufferSizeInvalid.text"));
						invalidFieldsMessage
								.append(FollowApp.MESSAGE_LINE_SEPARATOR);
						invalidFieldsMessage
								.append(FollowApp.MESSAGE_LINE_SEPARATOR);
					}
			...
			// Close button
			close = new JButton(
					FollowApp.getResourceString("dialog.Configure.close.label"));
			close.setMnemonic(FollowApp.getResourceString(
	...
	private class CfgFontSelectionPanel extends FontSelectionPanel {
		CfgFontSelectionPanel() {
			super(Configure.this.getApp().getAttributes().getFont(),
					getStyleDisplayValues(), new int[] { 8, 9, 10, 12, 14 });
