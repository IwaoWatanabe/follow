...
import ghm.follow.jfc.ui.FileFollowingPane;
import ghm.follow.jfc.ui.SearchableTextPane;
import ghm.follow.jfc.ui.StartupStatus;
...
public class FollowApp {
	...
	private final HashMap<String, FollowAppAction> actions = new HashMap<String, FollowAppAction>();
	private SystemInterface systemInterface;
	private final StartupStatus startupStatus;

	private final FollowAppAttributes attributes;
	...
	FollowApp(List<String> filenames, File propertyFile) throws IOException,
		...
		attributes.addPropertyChangeListener(
				FollowAppAttributes.RECENT_FILES_KEY, rfl);
		attributes.addPropertyChangeListener(
				FollowAppAttributes.RECENT_FILES_MAX_KEY, rfl);
		...
		// initialize menubar
		JMenuBar jMenuBar = ComponentBuilder.buildMenuBar(resources,
				getActions());
		...
		for (File file : files) {
			try {
				...
				if (nonexistentFilesBuffer == null) {
					nonexistentFilesBuffer = new StringBuffer(
		...
		if (nonexistentFileCount > 0) {
			...
			String message = MessageFormat.format(text, new Object[] {
					nonexistentFileCount, nonexistentFilesBuffer.toString() });
			// String title =
	...
	static ImageIcon getIcon(Class<?> clazz, String iconNameKey) {
		...
		if (url != null) {
			...
			LOG.finer("errored: "
					+ (java.awt.MediaTracker.ERRORED == icon
							.getImageLoadStatus()));
		}
		return icon;
	}
	 ...
	 * dispatching thread.
	 */
	void openFile(File file, boolean startFollowing)
		...
		FileFollowingPane fileFollowingPane = fileToFollowingPaneMap.get(file);
		if (fileFollowingPane != null) {
			// File is already open; merely select its tab
			tabbedPane.setSelectedComponent(fileFollowingPane);
		} else {
			fileFollowingPane = new FileFollowingPane(file,
					attributes.getBufferSize(), attributes.getLatency(),
					attributes.autoScroll(), attributes.getFont(),
					attributes.getTabSize());
...
//			addTitleAndCloseButton(tabbedPane);
	...
	private void updateActions() {
		...
		for (FollowAppAction a : actions.values()) {
			if (tabCount <= 1 && a.getContext() == FollowAppAction.ActionContext.MULTI_FILE) {
				a.setEnabled(false);
			} else if (tabCount == 0
					&& a.getContext() == FollowAppAction.ActionContext.SINGLE_FILE) {
				a.setEnabled(false);
			} else {
				a.setEnabled(true);
			}
	...
	List<FileFollowingPane> getAllFileFollowingPanes() {
		...
		for (int i = 0; i < tabCount; i++) {
			allFileFollowingPanes.add((FileFollowingPane) tabbedPane
					.getComponentAt(i));
		}
			...
			SwingUtilities.invokeAndWait(new Runnable() {
				public void run() {
					...
					instance.startupStatus
							.markDone(instance.startupStatus.CREATE_WIDGETS);
				}
			});
		...
		public void propertyChange(PropertyChangeEvent evt) {
			if (recentFilesMenu != null) {
				...
				for (int i = recentFiles.size() - 1; i >= 0; i--) {
					...
					// context of
					// the inner class
					recentFilesMenu.add(new Open(FollowApp.this, recentFiles
							.get(i)));
				}
			}
		}
