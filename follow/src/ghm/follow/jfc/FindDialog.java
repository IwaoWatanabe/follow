...
import ghm.follow.LineResult;
import ghm.follow.SearchEngine;
import ghm.follow.WordResult;
...
public class FindDialog extends JDialog {
	...
	public FindDialog(Find find) {
		super(find.getApp().getFrame(), FollowApp
				.getResourceString("dialog.Find.title"), false);
		...
		gbc.ipadx = 4;
		findPanel.add(
				new JLabel(FollowApp
						.getResourceString("dialog.Find.findText.label")), gbc);
		...
		caseSensitive = new JCheckBox(
				FollowApp.getResourceString("dialog.Find.caseSensitive.label"));
		gbc.gridx = 1;
		gbc.gridy = 1;
		gbc.weightx = 0;
		...
		regEx = new JCheckBox(
				FollowApp
						.getResourceString("dialog.Find.regularExpression.label"));
		gbc.gridx = 1;
		gbc.gridy = 2;
		gbc.weightx = 0;
		gbc.fill = GridBagConstraints.NONE;
		...
		findButton = new JButton(
				FollowApp.getResourceString("dialog.Find.findButton.label"));
		findButton.setMnemonic(FollowApp.getResourceString(
		...
		clearButton = new JButton(
				FollowApp.getResourceString("dialog.Find.clearButton.label"));
		clearButton.setMnemonic(FollowApp.getResourceString(
		...
		closeButton = new JButton(
				FollowApp.getResourceString("dialog.Find.closeButton.label"));
		closeButton.setMnemonic(FollowApp.getResourceString(
	...
	private void clearButton_clicked(ActionEvent e) {
		...
		FileFollowingPane pane = findAction.getApp()
	...
	private void resultList_changed(ListSelectionEvent ev) {
		if (!ev.getValueIsAdjusting()) {
			...
			if (pos >= 0) {
				...
				// selected position
				LineResult result = (LineResult) list.getModel().getElementAt(
						pos);

				// get the current selected tab
				// and text area
				FileFollowingPane ffp = findAction.getApp()
						.getSelectedFileFollowingPane();
	...
	private List<LineResult> doFind() {
		...
		List<LineResult> results = textArea.highlight(findField.getText(),
				flags);
