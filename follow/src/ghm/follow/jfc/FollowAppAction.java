...
abstract class FollowAppAction extends AbstractAction {
	...
	public FollowAppAction(FollowApp app, String name, String mnemonic,
			String accelerator, ActionContext type) {
		super(name);
		init(app, mnemonic, accelerator, type);
	}

	public FollowAppAction(FollowApp app, String name, String mnemonic,
			String accelerator, ImageIcon icon, ActionContext type) {
		super(name, icon);
		init(app, mnemonic, accelerator, type);
	}
	...
	void setAccelerator(String accelerator) throws InvalidVkException {
		if (accelerator != null && accelerator.length() > 0) {
			setAccelerator(KeyStroke.getKeyStroke(findKeyEventVk(accelerator),
					KeyEvent.CTRL_MASK));
		}
	}
	...
	private int findKeyEventVk(String key) throws InvalidVkException {
		...
		try {
			...
			throw new InvalidVkException(
					"Unable to match mnemonic to a field in KeyEvent", e);
