...
public class FileFollowingPane extends JScrollPane {
	...
	public FileFollowingPane(File file, int bufferSize, int latency,
			boolean autoPositionCaret, Font font, int tabSize) {
		...
		synchronized (fileFollower) {
			try {
				...
				log.log(Level.SEVERE,
						"InterrupedException in FileFollowingPane",
						interruptedException);
			}

			// This has the effect of clearing the contents of the followed file
			BufferedOutputStream bos = new BufferedOutputStream(
					new FileOutputStream(fileFollower.getFollowedFile()));
			bos.close();
			...
			try {
				...
				log.log(Level.WARNING,
						"BadLocationException in FileFolloingPane", e);
			}
