...
public class FontSelectionPanel extends JPanel {
	 ...
	 * that a default list of styles (
	 * <code>{"Plain", "Bold", "Italic", "Bold Italic"}</code>) and font sizes (
	 * <code>{8, 9, 10, 12, 14}</code>) will be used.
	 * 
	 * @param initialFont
	 *            see
	...
	public FontSelectionPanel(Font initialFont) {
		this(
				initialFont,
				// Don't change the following two values without changing the
				// javadocs
	...
	public FontSelectionPanel(Font initialFont, String[] styleDisplayNames,
		...
		phraseCanvas = new PhraseCanvas(initialFont.getFamily(), initialFont,
				Color.black);
		addObserver(new Observer() {
			public void update(Observable o, Object arg) {
				try {
					phraseCanvas.setPhrase((String) fontFamilyList
							.getSelectedValue());
					phraseCanvas.setFont(FontSelectionPanel.this
							.getSelectedFont());
	...
	public String getSelectedFontFamily() throws NoFontFamilySelectedException {
		...
		if (fontFamily == null) {
			throw new NoFontFamilySelectedException(
	 ...
	 */
	public int getSelectedFontSize() throws NoFontSizeSpecifiedException,
	...
	public void setSelectedFontFamily(String family) {
		...
		throw new IllegalArgumentException("The font family supplied, '"
				+ family + "', is not in the list of availalbe "
				+ "font families.");
	}
	...
	protected class FontSizeSynchronizer implements DocumentListener,
		...
		protected void handle(DocumentEvent e) {
			...
			updating_ = true;
			try {
				Integer currentFontSizeInteger = Integer.valueOf(textField_
						.getText());
	...
	static class FontStyleList extends JList {
		...
		public int getSelectedStyle() throws NoFontStyleSelectedException {
			...
			default:
				throw new NoFontStyleSelectedException(
	...
	static class PhraseCanvas extends Canvas {
		...
		public void paint(Graphics g) {
			...
			Font dummyFont = new Font(font_.getFamily(), font_.getStyle(),
					font_.getSize() + 1);
			dummyFont.createGlyphVector(new FontRenderContext(null,
					antialiasOn_, false), phrase_);

			GlyphVector glyphVector = font_.createGlyphVector(
					new FontRenderContext(null, antialiasOn_, false), phrase_);
