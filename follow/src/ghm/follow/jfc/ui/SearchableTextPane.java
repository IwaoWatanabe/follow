...
public class SearchableTextPane extends JTextArea {
	...
	private final DefaultHighlightPainter wordPainter = new DefaultHighlightPainter(
			Color.YELLOW);
	...
	public List<LineResult> highlight(String term, int flags) {
		...
		if ((term != null) && (term.length() > 0)) {
			...
			try {
				...
				List<WordResult> searchResults = new SearchEngine(flags)
						.search(term, text);
				...
				log.log(Level.SEVERE,
						"BadLocationException in SearchableTextPane", e);
	 ...
	 * @return
	 */
	private List<LineResult> convertWords2Lines(List<WordResult> words)
			throws BadLocationException {
