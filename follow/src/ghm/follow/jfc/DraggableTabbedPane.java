...
import ghm.follow.jfc.FollowAppAttributes;
...
/**
 * Tabbed pane for draggable tabs.
 *
...
public class DraggableTabbedPane extends TabbedPane {
	...
	public DraggableTabbedPane(FollowAppAttributes attributes) {
		super(attributes);
		...
		addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent e) {
				...
				if (dragging) {
					...
					// if we're not on a tab, see if we're outside the list
					if (tabNumber < 0) {
						Component firstTab = getComponentAt(0);
						Component lastTab = getComponentAt(getTabCount() -1);
						
						if (e.getX() < firstTab.getX()) {
							tabNumber = 0;
						} else if (e.getX() > lastTab.getX()) {
