...
public class JTextComponentDestinationT extends BaseTestCase {
	@Override
	@Before
	...
	public void testNoAutoscroll() throws Exception {
		validateTextContentAndCaretPos(new JTextComponentDestination(jTextArea,
				false));
	}
	...
	private void validateTextContentAndCaretPos(
			JTextComponentDestination destination) throws Exception {
	...
	private void validateCaretPosition(JTextComponentDestination destination) {
		...
		if (destination.autoPositionCaret()) {
			assertEquals(destination.getJTextComponent().getText().length(),
					caretPos);
		} else {
